...
<p class="form-field">
    ...
    <select multiple class="select2-hidden-accessible wc4bp-group-search" style="width: 50%;" id="wc4bp-group-ids"
        name="wc4bp-group-ids"
        data-placeholder="<?php wc4bp_groups_manager::echo_esc_attr_translation('Search for a group'); ?>"
        data-action="wc4bp_group_search" data-multiple="true" data-exclude="<?php echo intval($post->ID); ?>"
...
if ($type === 'variable' && $product instanceof WC_Product_Variable) {
	...
	echo esc_html(rtrim(trim($wc4bp_groups_available_variation), ','));
}
...
?>" value="<?php echo esc_attr(implode(',', array_keys($json_ids))); ?>"></select>
    <button type="button"
        class="button wc4bp add_groups"><?php wc4bp_groups_manager::echo_translation('Add Group'); ?></button>
