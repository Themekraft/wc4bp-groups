...
class wc4bp_groups_model
{
	...
	public function get_group_view()
	{
		...
		if (!empty($_POST['group']) && wp_doing_ajax() && is_string($_POST['group'])) {
			...
			if (is_object($data)) {
				...
				$str = ob_get_clean();
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Output is from controlled template file.
				echo "$str";
			}
		}
	...
	public static function add_member_to_group($group_id, $user_id = false, $is_admin = 0, $is_mod = 0)
	{
		...
		if (bp_is_active('activity')) {
			// translators: 1: User link, 2: Group name with link.
			groups_record_activity(array(
				'user_id' => $user_id,
				'action' => apply_filters(
					'groups_activity_joined_group',
					sprintf(
						__('%1$s joined the group %2$s', 'buddyforms'),
						bp_core_get_userlink($user_id),
						'<a href="' . bp_get_group_permalink($bp->groups->current_group) . '">' . esc_html($bp->groups->current_group->name) . '</a>'
					)
				),
				'type' => 'joined_group',
				'item_id' => $group_id,
