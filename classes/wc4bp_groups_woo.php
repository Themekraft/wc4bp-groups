...
class wc4bp_groups_woo extends wc4bp_groups_woo_base
{
	...
	public function add_after_oder_item_meta($item_id, $item, $_product)
	{
		...
		if (isset($item['wc4bp_groups'])) {
			...
			echo '<tr><th>' . esc_html(wc4bp_groups_manager::translation('BuddyPress Group(s)')) . ':</th><td>' . esc_html($groups_str) . '</td></tr>';
			echo '</table>';
		}
	}
	...
	public function output_checkbox($field)
	{
		...
		echo '<div ' . esc_attr($is_variation) . ' class="form-field ' . esc_attr($field['id']) . '_field ' . esc_attr($field['wrapper_class']) . '"><legend>' . wp_kses_post($field['label']) . '</legend><ul class="wc4bp-group-radios">';
		...
		if (!empty($field['description'])) {
			if (isset($field['desc_tip']) && false !== $field['desc_tip']) {
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- wc_help_tip() escapes internally via esc_attr().
				echo wc_help_tip(esc_html($field['description']));
			} else {
